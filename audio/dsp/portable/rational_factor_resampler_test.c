...
void TestCompareWithReferenceResampler(float filter_radius_factor) {
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      CHECK(abs(output_size - expected_size) <= 2);
...
void TestStreamingRandomBlockSizes() {
  ...
  float* input = CHECK_NOTNULL(malloc(sizeof(float) * kTotalInputSize));
  int n;
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      for (n = 0; n < kTotalInputSize;) {
        ...
        CHECK(expected_output_block_size == output_block_size);

        /* Append output_block to the `streaming` array. */
        CHECK(streaming_size + output_block_size <= total_output_size);
      ...
      CHECK(n == kTotalInputSize);
      CHECK(streaming_size == total_output_size);
