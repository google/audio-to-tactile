 ...
 *
 * These bindings wrap the resampler.c library in audio/dsp/portable as a
...
static int ResamplerImplObjectInit(ResamplerImplObject* self,
  ...
  if (!PyArg_ParseTupleAndKeywords(
          args, kw, "ffiidfffi:__init__", (char**)keywords,
          &input_sample_rate_hz,
          &output_sample_rate_hz,
...
static PyObject* ResamplerImplObjectProcessSamples(
  ...
  const int samples_size = PyArray_SIZE(samples);
  RationalFactorResampler* resampler = self->resampler;

  /* Create output numpy array. */
  npy_intp output_size = RationalFactorResamplerNextOutputSize(
      resampler, samples_size);
  ...
  /* Process the samples. */
  const int max_input_size = RationalFactorResamplerMaxInputSize(resampler);
  const float* samples_data = (const float*)PyArray_DATA(samples);
  float* output_data = (float*)PyArray_DATA(output);
  int start;
  for (start = 0; start < samples_size; start += max_input_size) {
    int block_input_size = samples_size - start;
    if (max_input_size < block_input_size) {
      block_input_size = max_input_size;
    }
    const int block_output_size = RationalFactorResamplerProcessSamples(
        resampler, samples_data + start, block_input_size);
    const float* block_output = RationalFactorResamplerOutput(resampler);
    memcpy(output_data, block_output, sizeof(float) * block_output_size);
    output_data += block_output_size;
  }
...
/* Define `ResamplerImpl.flush_size` property getter. */
static PyObject* ResamplerImplObjectFlushSize(ResamplerImplObject* self) {
  return Py_BuildValue("i", RationalFactorResamplerFlushSize(self->resampler));
}
...
static PyGetSetDef kResamplerImplGetSetDef[] = {
    ...
    {"flush_size", (getter)ResamplerImplObjectFlushSize,
     NULL, "Number of input samples needed to flush the reampler."},
    {NULL} /* Sentinel */
};
...
static /*bool*/ int InitModule(PyObject* m) {
  return AddTypeToModule(m, &kResamplerImplType) &&
...
PyMODINIT_FUNC initrational_factor_resampler_python_bindings() {
  import_array();
  PyObject* m = Py_InitModule("rational_factor_resampler_python_bindings",
                              kModuleMethods);
  InitModule(m);
}
#endif
