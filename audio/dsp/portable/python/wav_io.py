...
This library uses the read_wav_file_generic.c and write_wav_file_generic.c in
//audio/dsp/portable for WAV reading and writing.
...
import io
from typing import Any, Iterable, Text, Tuple, Union

import numpy as np
import six

from audio.dsp.portable.python import wav_io_python_bindings


def read_wav_file(filename, dtype=None):
  # type: (Text, Union[None, np.dtype]) -> Tuple[np.ndarray, int]
...
def write_wav_file(filename, samples, sample_rate_hz):
  # type: (Text, Iterable[Any], int) -> None
...
def from_bytes(wav_bytes, dtype=None):
  # type: (bytes, Union[None, np.dtype]) -> Tuple[np.ndarray, int]
...
def to_bytes(samples, sample_rate_hz):
  # type: (Iterable[Any], int) -> bytes
