 ...
 *
 *
 * Python bindings for //audio/dsp/portable WAV reading and writing.
 *
 * These bindings wrap the read_wav_file_generic.c and write_wav_file_generic.c
 * libraries in audio/dsp/portable as a `wav_io` Python module implementing
...
static int FlushWriteBuffer(FileObjectWithBuffer* f) {
  ...
  PyObject* write_result = PyObject_CallMethod(
      f->file_object, "write", (PY_MAJOR_VERSION >= 3) ? "y#" : "s#",
      f->buffer, count);
...
PyMODINIT_FUNC PyInit_wav_io_python_bindings() {
  import_array();
  return PyModule_Create(&kModule);
}
#else
PyMODINIT_FUNC initwav_io_python_bindings() {
  import_array();
  PyObject* m = Py_InitModule("wav_io_python_bindings", kModuleMethods);
}
#endif
