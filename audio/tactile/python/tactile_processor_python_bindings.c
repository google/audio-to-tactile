 ...
 *
 * These bindings wrap the tactile_processor.c library in audio/tactile as a
...
#else
PyMODINIT_FUNC inittactile_processor() {
  import_array();
  PyObject* m = Py_InitModule("tactile_processor", kModuleMethods);
  InitModule(m);
}
#endif
