...
CarlFrontend* CarlFrontendMake(const CarlFrontendParams* params) {
  ...
  for (c = 0; c < num_channels; ++c) {
    ...
    const double peak_gain = CarlFrontendFindPeakGain(
        ...
        &peak_frequency_hz);
    frontend->channel_data[c].biquad_b0 /= peak_gain;
    frontend->channel_data[c].biquad_b1 /= peak_gain;
    frontend->channel_data[c].biquad_b2 /= peak_gain;
...
void CarlFrontendReset(CarlFrontend* frontend) {
  ...
  for (c = 0; c < frontend->num_channels; ++c) {
    ...
    channel_state->biquad_state0 = 0.0f;
    channel_state->biquad_state1 = 0.0f;
...
void CarlFrontendProcessSamples(CarlFrontend* frontend,
  ...
  for (c = 0; c < num_channels; ++c) {
    ...
    for (i = 0; i < frontend->block_size; i += stride) {
      ...
      const float next_biquad_state = input[i]
          - channel_data.biquad_a1 * channel_state.biquad_state0
          - channel_data.biquad_a2 * channel_state.biquad_state1;
      const float biquad_output =
          channel_data.biquad_b0 * next_biquad_state
          + channel_data.biquad_b1 * channel_state.biquad_state0
          + channel_data.biquad_b2 * channel_state.biquad_state1;
      channel_state.biquad_state1 = channel_state.biquad_state0;
      channel_state.biquad_state0 = next_biquad_state;
