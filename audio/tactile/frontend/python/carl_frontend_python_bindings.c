 ...
 *
 * These bindings wrap the carl_frontend.c library in audio/tactile/frontend as
 * a `carl_frontend` Python module containing a `CarlFrontend` Python class.
 *
 ...
 *      audio/tactile/frontend. See that file for details.
 *    Raises:
...
static PyObject* CarlFrontendObjectNumChannels(CarlFrontendObject* self) {
  return MakePyInteger(CarlFrontendNumChannels(self->frontend));
}

/* Define `CarlFrontend.block_size` property getter. */
static PyObject* CarlFrontendObjectBlockSize(CarlFrontendObject* self) {
  return MakePyInteger(CarlFrontendBlockSize(self->frontend));
}
...
PyMODINIT_FUNC PyInit_carl_frontend() {
  import_array();
...
#else
PyMODINIT_FUNC initcarl_frontend() {
  import_array();
  PyObject* m = Py_InitModule("carl_frontend", kModuleMethods);
  DefineCarlFrontendType(m);
}
#endif
