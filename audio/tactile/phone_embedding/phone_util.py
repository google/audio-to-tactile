...
from typing import Any, Callable, Dict, Iterable, List, Text, TypeVar, Tuple
...
def _map_dict_values(fun, d):
  # type: (Callable[[VT_in], VT_out], Dict[KT, VT_in]) -> Dict[KT, VT_out]
  return {k: fun(v) for k, v in six.iteritems(d)}


def get_main_module_flags_dict():
  # type: () -> Dict
...
def get_phone_label_filename(wav_file):
  # type: (Text) -> Text
  """Gets the phone label filename associated with `wav_file`."""
  return os.path.splitext(wav_file)[0] + '.phn'


def get_phone_times(phn_file):
  # type: (Any) -> List[Tuple[int, int, Text]]
  ...
  def _read(f):
    ...
    try:
      ...
      raise IOError('Error reading .phn file %s: %s' % (name, e))

  if isinstance(phn_file, six.string_types):
...
def run_frontend(frontend, audio_samples):
  # type: (carl_frontend.CarlFrontend, np.ndarray) -> np.ndarray
  ...
  padding = (-len(audio_samples)) % frontend.block_size
  audio_samples = np.append(audio_samples, np.zeros(padding, np.float32))
  return frontend.process_samples(audio_samples)


ItemT = TypeVar('ItemT')


def run_in_parallel(items, num_threads, fun):
  # type: (Iterable[ItemT], int, Callable[[ItemT], Any]) -> None
...
def balance_weights(example_counts,
                    fg_classes,
                    fg_balance_exponent,
                    bg_fraction,
                    bg_balance_exponent):
  # type: (Dict[Text, int], Iterable, float, float, float) -> Dict[Text, float]
...
class Dataset(object):
  ...
  def __init__(self, examples, metadata):
    # type: (Dict[Text, np.ndarray], Dict) -> None
    """Constructor.
  ...
  def _validate(self):
    ...
    for k, v in six.iteritems(self.examples):
      if not isinstance(v, np.ndarray):
        raise TypeError('"%s": expected numpy array, %s found' % (k, type(v)))
      elif v.shape[1:] != (self.num_frames, self.num_channels):
        raise ValueError(
            '"%s": shape %s mismatches expected shape Nx%dx%d' %
            (k, v.shape, self.num_frames, self.num_channels))
      elif not np.all(np.isfinite(v)):
        raise ValueError('"%s": array has nonfinite value' % k)

  def _drop_empty_classes(self):
    # type: () -> None
    """Drop empty classes from `self.examples`."""
    self.examples = {k: v for k, v in six.iteritems(self.examples) if len(v)}
  ...
  @property
  def example_counts(self):
    # type: () -> Dict[Text, int]
    return _map_dict_values(len, self.examples)

  def get_xy_arrays(self, phones, shuffle=False):
    # type: (Iterable[Text], bool) -> Tuple[np.ndarray, np.ndarray]
  ...
  def subsample(self, fraction):
    ...
    for phone in self.examples:
      ...
      if not 0.0 <= fraction_phone <= 1.0:
        raise ValueError('fraction["%s"] = %g is not between 0.0 and 1.0'
                         % (phone, fraction_phone))
  ...
  def split(self, fraction):
    # type: (float) -> Tuple[Dataset, Dataset]
    ...
    if not 0.0 <= fraction <= 1.0:
      raise ValueError('fraction = %g is not between 0.0 and 1.0' % fraction)
...
def read_dataset_npz(npz_file):
  ...
  examples = {k: v.astype(np.float32) for k, v in six.iteritems(contents)
