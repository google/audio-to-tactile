 ...
 * audio/tactile/phone_embedding as a `embed_vowel` Python module containing a
...
#include "audio/tactile/phone_embedding/embed_vowel.h"
...
#else
PyMODINIT_FUNC initembed_vowel() {
  import_array();
  PyObject* m = Py_InitModule("embed_vowel", kModuleMethods);
  InitModule(m);
}
#endif
