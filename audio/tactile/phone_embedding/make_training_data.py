...
Make phone analysis training or testing examples from labeled data.

This program processes .wav recordings with .phn phone labels using the
CARL+PCEN frontend and writes a numpy .npz file.

Phone labels are read from .phn files in simple text format, as used in TIMIT.
Each row gives start and end sample indices and label for one phone, e.g.
...
# this since right context adds latency. A particular goal of this project is to
...
flags.DEFINE_float('min_phone_length_s', 0.05,
                   'Min phone length in seconds. Segments shorter than this '
                   'are skipped.')
...
# Phones to exclude from the dataset. We skip the diphthong vowels, which acts
# like a transition between two monophthongs. We don't know from phone labeling
# when within the segment that transition occurs, so classification is unclear.
PHONES_TO_EXCLUDE_FROM_DATASET = {'aw', 'oy', 'ey', 'ay', 'ow', 'ax'}


def get_frontend_params_from_flags():
  # type: () -> Dict[Text, Any]
  return {
...
def process_one_wav_file(wav_file):
  ...
  FLAGS.min_phone_length_s and segments with diphthong vowel labels are skipped.
  ...
  for draw_index in range(FLAGS.num_draws):
    ...
    for start, end, phone in phone_times:
      start = int(round(dilation_factor * start))
      end = min(int(round(dilation_factor * end)), len(samples))
      ...
      # near the phone endpoints, so trim a fraction from each end.
      length = end - start
      ...
      examples[phone].append(dsp.sliding_window(
...
def process_wav_files(wav_files):
  # type: (Set[Text]) -> Dict[Text, np.ndarray]
...
def balance_examples(dataset):
  # type: (phone_util.Dataset) -> None
...
def main(_):
  ...
  if not wav_files:
    print('Error: No .wav files found matching %s' % FLAGS.examples)
    return 1

  for wav_file in wav_files:
    phn_file = phone_util.get_phone_label_filename(wav_file)
    if not os.path.isfile(phn_file):
      print('Error: .phn file not found: %s' % phn_file)
      return 1
  ...
  print('\nWriting dataset to %s' % FLAGS.output)
