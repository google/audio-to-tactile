...
void CarlFrontendDesignBiquad(double pole_frequency_hz, double sample_rate_hz,
  ...
  channel_data->biquad_coeffs.b0 = (float)(z_numerator[0] / z_denominator[0]);
  channel_data->biquad_coeffs.b1 = (float)(z_numerator[1] / z_denominator[0]);
  channel_data->biquad_coeffs.b2 = (float)(z_numerator[2] / z_denominator[0]);
  channel_data->biquad_coeffs.a1 = (float)(z_denominator[1] / z_denominator[0]);
  channel_data->biquad_coeffs.a2 = (float)(z_denominator[2] / z_denominator[0]);
}
...
static ComplexDouble BiquadFrequencyResponse(
  ...
  ComplexDouble numerator = ComplexDoubleMul(
      ComplexDoubleMake(channel_data->biquad_coeffs.b0 * z.real +
                            channel_data->biquad_coeffs.b1,
                        channel_data->biquad_coeffs.b0 * z.imag), z);
  numerator.real += channel_data->biquad_coeffs.b2;
  /* denominator = (z + a1) * z + a2. */
  ComplexDouble denominator = ComplexDoubleMul(
      ComplexDoubleMake(z.real + channel_data->biquad_coeffs.a1, z.imag), z);
  denominator.real += channel_data->biquad_coeffs.a2;
