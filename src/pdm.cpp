...
void PdmMic::Initialize(uint16_t clock_pin, uint16_t data_pin) {
  ...
  nrf_pdm_psel_connect(clock_pin, data_pin);

  // initialize and connect to interrupt handler.
  nrf_pdm_int_enable(NRF_PDM_INT_STARTED | NRF_PDM_INT_END |
                     NRF_PDM_INT_STOPPED);
...
void PdmMic::SetMicGain(int16_t gain) { nrf_pdm_gain_set(gain, gain); }
...
void PdmMic::IrqHandler() {
  ...
  if (nrf_pdm_event_check(NRF_PDM_EVENT_STARTED)) {
    ...
    const int i = (nrf_pdm_buffer_get() == (uint32_t *)pdm_buffer_[0]) ? 1 : 0;
    nrf_pdm_buffer_set((uint32_t *)pdm_buffer_[i], kPdmDataSize);
    which_buffer_ready_ = i;
  }

  // PDM module is stopped.
  if (nrf_pdm_event_check(NRF_PDM_EVENT_STOPPED)) {
