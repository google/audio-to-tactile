...
#include "src/dsp/fast_fun_compute_tables.h"
...
static void TestFastExp2Accuracy(void) {
  ...
  CHECK(max_rel_error < kThreshold);
}
...
static void TestFastLog2DerivativeAccuracy(void) {
  ...
  CHECK(dlog2_max_rel_error < 0.04);
}
...
static void TestFastExp2DerivativeAccuracy(void) {
  ...
  CHECK(dexp2_max_rel_error < 0.04);
}
...
static void TestCheckTables(void) {
  ...
  int i;
  {
    ...
    for (i = 0; i < 256; ++i) {
      CHECK(fabs(table[i] - kFastFunLog2Table[i]) <= 1e-8);
    }
  }
  {
    int32_t table[256];
    ...
    for (i = 0; i < 256; ++i) {
      CHECK(abs(table[i] - kFastFunExp2Table[i]) <= 1);
    }
  }
}
