...
void TestKernel(float filter_radius_factor) {
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      CHECK(QResamplerKernelInit(&kernel, input_sample_rate_hz,
                                 output_sample_rate_hz, filter_radius_factor,
                                 kCutoff, kBeta));
       ...
       */
      CHECK(fabs(QResamplerKernelEval(&kernel, -kernel.radius)) > 1e-6);
      CHECK(fabs(QResamplerKernelEval(&kernel, kernel.radius)) > 1e-6);
      /* The kernel is zero outside of [-radius, +radius]. */
      CHECK(QResamplerKernelEval(&kernel, -kernel.radius - 1e-6) == 0.0);
      CHECK(QResamplerKernelEval(&kernel, kernel.radius + 1e-6) == 0.0);

      const double value_0 = QResamplerKernelEval(&kernel, 0.0);
      const double dx = 0.02;
      double sum = 0.0;
      double x;
      for (x = dx; x <= kernel.radius; x += dx) {
        double value_x = QResamplerKernelEval(&kernel, x);
