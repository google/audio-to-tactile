...
static void TestZpkEval(void) {
  ...
  for (trial = 0; trial < 5; ++trial) {
    ...
    CHECK(ComplexEqual(result, expected));
  }
}
...
static void TestZpkSort(void) {
  ...
  CHECK(ZpkSort(&zpk));
  CHECK(zpk.num_zeros == 8);
  CHECK(zpk.num_poles == 6);
  ...
  int n;
  for (n = 0; n < 8; ++n) {
    CHECK(ComplexEqual(zpk.zeros[n], expected_zeros[n]));
  }
  /* Poles: s = -3 +/- i, -2 +/- i, -1 +/- i. */
  ...
  for (n = 0; n < 6; ++n) {
    CHECK(ComplexEqual(zpk.poles[n], expected_poles[n]));
  }
  ...
  for (trial = 0; trial < 20; ++trial) {
    ...
    ComplexDouble* expected = (ComplexDouble*)CHECK_NOTNULL(
    ...
    CHECK(ZpkSort(&zpk));  /* Put zpk.zeros back in sorted order. */

    for (n = 0; n < num_total; ++n) {
      CHECK(ComplexEqual(zpk.zeros[n], expected[n]));
    }
...
static void TestZpkBilinearTransform(void) {
  ...
  for (trial = 0; trial < 10; ++trial) {
    ...
    CHECK(ZpkBilinearTransform(&discretized, sample_rate_hz));
    ...
    for (n = 0; n < 10; ++n) {
      ...
      CHECK(ComplexNear(ZpkEval(&discretized, z), ZpkEval(&analog, s), 1e-9));
    }
  }
}
...
static void TestZpkToBiquads(void) {
  ...
  for (trial = 0; trial < 10; ++trial) {
    Zpk zpk;
    MakeRandomZpk(&zpk);
    CHECK(ZpkBilinearTransform(&zpk, 1.0));

    const int num_biquads = (zpk.num_poles + 1) / 2;
    BiquadFilterCoeffs* coeffs = (BiquadFilterCoeffs*)CHECK_NOTNULL(
        malloc(num_biquads * sizeof(BiquadFilterCoeffs)));

    CHECK(ZpkToBiquads(&zpk, coeffs, num_biquads) == num_biquads);
    ...
    for (n = 0; n < 10; ++n) {
      ...
      CHECK(ComplexNear(ZpkEval(&zpk, z),
...
static void TestButterworthLowpassCompareWithScipy(void) {
  ...
  CHECK(DesignButterworthLowpass(2, 2800.0, 8000.0, coeffs, 1) == 1);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected1, 1));

  CHECK(DesignButterworthLowpass(2, 6700.0, 44100.0, coeffs, 1) == 1);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected2, 1));

  CHECK(DesignButterworthLowpass(3, 2800.0, 8000.0, coeffs, 2) == 2);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected3, 2));

  CHECK(DesignButterworthLowpass(3, 6700.0, 44100.0, coeffs, 2) == 2);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected4, 2));

  CHECK(DesignButterworthLowpass(8, 2800.0, 8000.0, coeffs, 4) == 4);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected5, 4));

  CHECK(DesignButterworthLowpass(8, 6700.0, 44100.0, coeffs, 4) == 4);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected6, 4));
}
...
static void TestButterworthHighpassCompareWithScipy(void) {
  ...
  CHECK(DesignButterworthHighpass(2, 2800.0, 8000.0, coeffs, 1) == 1);
  /* Compare with scipy.signal.butter(2, 2800 / 4000.0, btype='high'). */
  static const BiquadFilterCoeffs kExpected1[] =  {
    {0.13110644f, -0.26221288f, 0.13110644f, 0.74778918f, 0.27221494f}};
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected1, 1));

  CHECK(DesignButterworthHighpass(3, 2800.0, 8000.0, coeffs, 2) == 2);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected2, 2));

  CHECK(DesignButterworthHighpass(8, 2800.0, 8000.0, coeffs, 4) == 4);
  /* Compare with scipy.signal.butter. */
  static const BiquadFilterCoeffs kExpected3[] =  {
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected3, 4));
}
...
static void TestButterworthBandpassCompareWithScipy(void) {
  ...
  CHECK(DesignButterworthBandpass(1, 400.0, 2000.0, 8000.0, coeffs, 1) == 1);
  /* Compare with butter(1, [400./4000, 2000./4000], btype='band'). */
  static const BiquadFilterCoeffs kExpected1[] =  {
    {0.42080778f, 0.0f, -0.42080778f, -0.84161556f, 0.15838444f}};
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected1, 1));

  CHECK(DesignButterworthBandpass(2, 1000.0, 2800.0, 8000.0, coeffs, 2) == 2);
  ...
  CHECK(DesignButterworthBandpass(3, 1000.0, 2800.0, 8000.0, coeffs, 3) == 3);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected3, 3));

  CHECK(DesignButterworthBandpass(8, 1000.0, 2800.0, 8000.0, coeffs, 8) == 8);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected4, 8));
}
...
static void TestButterworthBandstopCompareWithScipy(void) {
  ...
  CHECK(DesignButterworthBandstop(2, 1000.0, 2800.0, 8000.0, coeffs, 2) == 2);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected1, 2));

  CHECK(DesignButterworthBandstop(3, 1000.0, 2800.0, 8000.0, coeffs, 3) == 3);
  ...
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected2, 3));
}
...
static void TestChebyshev1LowpassCompareWithScipy(void) {
  ...
  CHECK(DesignChebyshev1Lowpass(2, 0.25, 2800.0, 8000.0, coeffs, 1) == 1);
  /* Compare with scipy.signal.cheby1(2, 0.25, 2800 / 4000.0). */
  static const BiquadFilterCoeffs kExpected1[] =  {
    {0.6245025f, 1.249005f, 0.6245025f, 1.12761236f, 0.44334085f}};
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected1, 1));

  CHECK(DesignChebyshev1Lowpass(3, 0.25, 2800.0, 8000.0, coeffs, 2) == 2);
  ...
  CHECK(DesignChebyshev1Lowpass(8, 0.25, 2800.0, 8000.0, coeffs, 4) == 4);
...
static void TestChebyshev2LowpassCompareWithScipy(void) {
  ...
  CHECK(DesignChebyshev2Lowpass(2, 40.0, 2800.0, 8000.0, coeffs, 1) == 1);
  /* Compare with scipy.signal.cheby2(2, 40.0, 2800 / 4000.0). */
  static const BiquadFilterCoeffs kExpected1[] =  {
    {0.05930588f, 0.09135622f, 0.05930588f, -1.25779318f, 0.46776116f}};
  CHECK(BiquadsAreApproximatelyEqual(coeffs, kExpected1, 1));

  CHECK(DesignChebyshev2Lowpass(3, 40.0, 2800.0, 8000.0, coeffs, 2) == 2);
  ...
  CHECK(DesignChebyshev2Lowpass(8, 40.0, 2800.0, 8000.0, coeffs, 4) == 4);
...
static void TestEllipticLowpassCompareWithMathematica(void) {
  ...
  CHECK(DesignEllipticLowpass(3, 0.5, 40.0, 2800.0, 8000.0, coeffs, 2) == 2);
  ...
  CHECK(DesignEllipticLowpass(4, 0.5, 40.0, 1200.0, 8000.0, coeffs, 2) == 2);
  /* Compare with Mathematica:
  ...
  CHECK(DesignEllipticLowpass(8, 0.5, 40.0, 2800.0, 8000.0, coeffs, 4) == 4);
  /* Compare with Mathematica:
   *   fc = 2*8000*Tan[Pi*2800/8000];
