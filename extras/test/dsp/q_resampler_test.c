 ...
 * output sample rate.
 */
static float* ReferenceResampling(const QResamplerKernel* kernel,
  ...
  for (m = 0;; ++m) {
    ...
    CHECK(QResamplerKernelEval(kernel, n0 - (n_first - 1)) == 0.0);
    CHECK(QResamplerKernelEval(kernel, n0 - (n_last + 1)) == 0.0);
    ...
    if (m >= output_capacity) {
      ...
      output = (float*)CHECK_NOTNULL(
          realloc(output, sizeof(float) * output_capacity * num_channels));
    }
    ...
    for (c = 0; c < num_channels; ++c) {
      ...
      for (n = n_first; n <= n_last; ++n) {
        sum += (n < 0 ? 0.0 : input[n * num_channels + c]) *
               QResamplerKernelEval(kernel, n0 - n);
      }
...
static void TestCompareWithReferenceResampler(int num_channels,
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      QResampler* resampler = CHECK_NOTNULL(
          QResamplerMake(input_sample_rate_hz, output_sample_rate_hz,
                         num_channels, kInputFrames, &options));

      int num_output_frames =
          QResamplerProcessSamples(resampler, input, kInputFrames);

      QResamplerKernel kernel;
      CHECK(QResamplerKernelInit(
      ...
      QResamplerGetRationalFactor(resampler, &rational_factor_numerator,
                                  &rational_factor_denominator);
      ...
      const float* output = QResamplerOutput(resampler);
...
static void TestStreamingRandomBlockSizes(int num_channels) {
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      QResampler* resampler = CHECK_NOTNULL(
          QResamplerMake(input_sample_rate_hz, output_sample_rate_hz,
                         num_channels, kTotalInputFrames, NULL));
      const int total_output_frames =
          QResamplerProcessSamples(resampler, input, kTotalInputFrames);
      float* nonstreaming = CHECK_NOTNULL(
          malloc(/* Save the output. */
                 sizeof(float) * total_output_frames * num_channels));
      memcpy(nonstreaming, QResamplerOutput(resampler),
             sizeof(float) * total_output_frames * num_channels);
      QResamplerFree(resampler);

      /* Do "streaming" resampling, passing successive blocks of input. */
      float* streaming = CHECK_NOTNULL(
          malloc(sizeof(float) * total_output_frames * num_channels));
      float* input_block =
          CHECK_NOTNULL(malloc(sizeof(float) * kMaxBlockFrames * num_channels));
      resampler = CHECK_NOTNULL(
          QResamplerMake(input_sample_rate_hz, output_sample_rate_hz,
                         num_channels, kMaxBlockFrames, NULL));
      CHECK(total_output_frames ==
            QResamplerNextNumOutputFrames(resampler, kTotalInputFrames));
      const int max_output_block_frames = QResamplerMaxOutputFrames(resampler);
      ...
      for (n = 0; n < kTotalInputFrames;) {
         ...
         */
        int input_block_frames =
            (int)floor((((float)rand()) / RAND_MAX) * kMaxBlockFrames + 0.5);
        ...
        const int expected_output_block_size =
            QResamplerNextNumOutputFrames(resampler, input_block_frames);
        const int output_block_frames = QResamplerProcessSamples(
            resampler, input_block, input_block_frames);
        float* output_block = QResamplerOutput(resampler);
...
static void TestResampleSineWave(void) {
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      QResampler* resampler = CHECK_NOTNULL(
          QResamplerMake(input_sample_rate_hz, output_sample_rate_hz,
                         /*num_channels=*/1, kInputSize, NULL));

      /* Run resampler on the sine wave samples. */
      const int output_size =
          QResamplerProcessSamples(resampler, input, kInputSize);
      const float* output = QResamplerOutput(resampler);
...
static void TestResampleChirp(void) {
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      QResampler* resampler = CHECK_NOTNULL(
          QResamplerMake(input_sample_rate_hz, output_sample_rate_hz,
                         /*num_channels=*/1, input_size, NULL));

      /* Run resampler on the chirp. */
      const int output_size =
          QResamplerProcessSamples(resampler, input, input_size);
      const float* output = QResamplerOutput(resampler);
      ...
      const float cutoff_hz =
          kernel.radians_per_sample * input_sample_rate_hz / (2 * M_PI);
...
static void TestInputSizeExceedsMax(int num_channels) {
  ...
  for (i = 0; i < kNumRates; ++i) {
    ...
    for (j = 0; j < kNumRates; ++j) {
      ...
      QResampler* resampler = CHECK_NOTNULL(
          QResamplerMake(input_sample_rate_hz, output_sample_rate_hz,
                         num_channels, kMaxInputFrames, NULL));

      /* Process the first 50 frames. */
      QResamplerProcessSamples(resampler, input, 50);
      ...
      const int num_output_frames =
          QResamplerProcessSamples(resampler, input + 50 * num_channels, 70);

      float* output_with_drop = CHECK_NOTNULL(
          malloc(/* Save the output. */
                 sizeof(float) * num_output_frames * num_channels));
      memcpy(output_with_drop, QResamplerOutput(resampler),
             sizeof(float) * num_output_frames * num_channels);

      /* Compare with explicitly resetting and processing the last 50 frames. */
      QResamplerReset(resampler);
      const int expected_size =
          QResamplerProcessSamples(resampler, input + 70 * num_channels, 50);
      const float* expected = QResamplerOutput(resampler);
