 ...
 * frequency response.
 */
static void TestLowpassResponse(void) {
  ...
  for (trial = 0; trial < 20; ++trial) {
    ...
    for (j = 0; j < 5; ++j) {
      ...
      CHECK(1 / M_SQRT2 - 1e-4 <= gain && gain <= 1 + 1e-4);
    }
    ...
    for (j = 1; j < 4; ++j) {
      ...
      gain = FrequencyResponse(&coeffs, 1, f, sample_rate_hz);
      CHECK(gain <= 1 / M_SQRT2 + 1e-4);
    }

    /* Zero gain at Nyquist. */
    f = sample_rate_hz / 2;
    gain = FrequencyResponse(&coeffs, 1, f, sample_rate_hz);
    CHECK(fabs(gain) <= 1e-4);
  }
}

/* Compare with lowpass filters designed by scipy.signal.butter. */
static void TestLowpassCompareWithScipy(void) {
  ...
  CHECK(fabs(coeffs.b0 - 0.50500103f) <= 5e-7f);
  CHECK(fabs(coeffs.b1 - 1.01000206f) <= 5e-7f);
  CHECK(fabs(coeffs.b2 - 0.50500103f) <= 5e-7f);
  CHECK(fabs(coeffs.a1 - 0.74778918f) <= 5e-7f);
  CHECK(fabs(coeffs.a2 - 0.27221494f) <= 5e-7f);

  CHECK(DesignButterworthOrder2Lowpass(6700.0, 44100.0, &coeffs));

  /* Compare with scipy.signal.butter(2, 6700 / 22050.0). */
  CHECK(fabs(coeffs.b0 - 0.13381137f) <= 5e-7f);
  CHECK(fabs(coeffs.b1 - 0.26762273f) <= 5e-7f);
  CHECK(fabs(coeffs.b2 - 0.13381137f) <= 5e-7f);
  CHECK(fabs(coeffs.a1 - -0.73294293f) <= 5e-7f);
  CHECK(fabs(coeffs.a2 - 0.26818839f) <= 5e-7f);
}

/* Check that invalid arguments are rejected. */
static void TestLowpassInvalidArgs(void) {
  puts("TestLowpassInvalidArgs");
  BiquadFilterCoeffs coeffs;
  CHECK(!DesignButterworthOrder2Lowpass(0.0, 1000.0, &coeffs));
  CHECK(!DesignButterworthOrder2Lowpass(900.0, 1000.0, &coeffs));
  CHECK(!DesignButterworthOrder2Lowpass(100.0, 1000.0, NULL));
}

/* Design highpass filters with randomly choosen parameters and check the
 * frequency response.
 */
static void TestHighpassResponse(void) {
  ...
  for (trial = 0; trial < 20; ++trial) {
    ...
    CHECK(DesignButterworthOrder2Highpass(
      corner_frequency_hz, sample_rate_hz, &coeffs) == 1);
    ...
    f = sample_rate_hz / 2;
    gain = FrequencyResponse(&coeffs, 1, f, sample_rate_hz);
    CHECK(fabs(gain - 1.0f) <= 1e-4);
  }
}

/* Compare with highpass filters designed by scipy.signal.butter. */
static void TestHighpassCompareWithScipy(void) {
  ...
  CHECK(fabs(coeffs.b0 - 0.13110644f) <= 5e-7f);
  CHECK(fabs(coeffs.b1 - -0.26221288f) <= 5e-7f);
  CHECK(fabs(coeffs.b2 - 0.13110644f) <= 5e-7f);
  CHECK(fabs(coeffs.a1 - 0.74778918f) <= 5e-7f);
  CHECK(fabs(coeffs.a2 - 0.27221494f) <= 5e-7f);

  CHECK(DesignButterworthOrder2Highpass(6700.0, 44100.0, &coeffs));

  /* Compare with scipy.signal.butter(2, 6700 / 22050.0, btype='high'). */
  CHECK(fabs(coeffs.b0 - 0.50028283f) <= 5e-7f);
  CHECK(fabs(coeffs.b1 - -1.00056566f) <= 5e-7f);
  CHECK(fabs(coeffs.b2 - 0.50028283f) <= 5e-7f);
  CHECK(fabs(coeffs.a1 - -0.73294293f) <= 5e-7f);
  CHECK(fabs(coeffs.a2 - 0.26818839f) <= 5e-7f);
}

/* Check that invalid arguments are rejected. */
static void TestHighpassInvalidArgs(void) {
  puts("TestHighpassInvalidArgs");
  BiquadFilterCoeffs coeffs;
  CHECK(!DesignButterworthOrder2Highpass(0.0, 1000.0, &coeffs));
  CHECK(!DesignButterworthOrder2Highpass(900.0, 1000.0, &coeffs));
  CHECK(!DesignButterworthOrder2Highpass(100.0, 1000.0, NULL));
}

/* Design bandpass filters with randomly choosen parameters and check the
 * frequency response.
 */
static void TestBandpassResponse(void) {
  ...
  for (trial = 0; trial < 20; ++trial) {
    ...
    CHECK(DesignButterworthOrder2Bandpass(
...
static void TestBandpassCompareWithScipy(void) {
  puts("TestBandpassCompareWithScipy");
  BiquadFilterCoeffs coeffs[2];
  float b[5];
  float a[5];

  CHECK(DesignButterworthOrder2Bandpass(1500.0, 2800.0, 8000.0, coeffs));
   ...
   */
  {
    ...
    int i;
    for (i = 0; i < 5; ++i) {
      CHECK(fabs(b[i] - scipy_b[i]) <= 5e-7f);
      CHECK(fabs(a[i] - scipy_a[i]) <= 5e-7f);
    }
  }

  CHECK(DesignButterworthOrder2Bandpass(80.0, 6700.0, 44100.0, coeffs));
...
/* Check that invalid arguments are rejected. */
static void TestBandpassInvalidArgs(void) {
  puts("TestBandpassInvalidArgs");
  BiquadFilterCoeffs coeffs[2];
  CHECK(!DesignButterworthOrder2Bandpass(-1.0, 100.0, 1000.0, coeffs));
  CHECK(!DesignButterworthOrder2Bandpass(400.0, 100.0, 1000.0, coeffs));
  CHECK(!DesignButterworthOrder2Bandpass(20.0, 900.0, 1000.0, coeffs));
  CHECK(!DesignButterworthOrder2Bandpass(20.0, 100.0, 1000.0, NULL));
}
