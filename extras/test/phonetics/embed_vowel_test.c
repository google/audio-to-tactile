...
#include "src/phonetics/embed_vowel.h"
...
void TestPhone(const char* phone) {
  ...
  int num_channels;
  int sample_rate_hz;
  int16_t* input_int16 = (int16_t*)CHECK_NOTNULL(
      Read16BitWavFile(wav_file, &num_samples, &num_channels, &sample_rate_hz));
  float* frame =
      (float*)CHECK_NOTNULL(malloc(sizeof(float) * kEmbedVowelNumChannels));
