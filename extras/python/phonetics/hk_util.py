...
def transform(f) -> hk.Transformed:
  """Wrapper for hk.transform() with post v0.0.2 behavior.

  In v0.0.1, apply_rng defaulted to False, but in later versions, the behavior
  defaults to True *and* the apply_rng arg was removed.

  Args:
    f: Function to be transformed.
  Returns:
    `hk.Transformed` tuple with init and apply functions.
  """
  if hk.__version__ == '0.0.1':
    return hk.transform(f, apply_rng=True)
  else:
...
class TrainedModel:
  ...
  def __call__(self, rng, inputs):
    ...
    return self.model_object.apply(self.params, rng, inputs)
  ...
  def load(cls,
    ...
    model_object = transform(functools.partial(model_fun, meta=meta))
  ...
  def _iterate_node(name, node):
    if isinstance(node, Mapping):
      for k in sorted(node.keys()):
        _iterate_node(name + '.' + k, node[k])
    else:
      out.append((name, node))

  for k in sorted(params.keys()):
    _iterate_node(k, params[k])
